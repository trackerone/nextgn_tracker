...
class AnnounceTest extends TestCase
{
    ...
    public function test_invalid_passkey_returns_failure(): void
    {
        ...
        $params = [
            'info_hash' => strtolower((string) $torrent->info_hash),  // 40-hex
            'peer_id' => $this->makePeerIdHex('invalid-passkey'),     // 40-hex
            'port' => 6881,
            'uploaded' => 0,
            'downloaded' => 0,
            'left' => 100,
    ...
    public function test_started_event_returns_success_payload(): void
    {
        ...
        $response = $this->announce($user, $torrent, [
        ...
        $this->assertStringContainsString('8:intervali1800e', (string) $response->getContent());
    }
    ...
    /**
     * Stable announce helper: always uses 40-hex transport for info_hash + peer_id.
     */
    private function announce(User $user, Torrent $torrent, array $overrides = []): TestResponse
    {
        $params = array_merge([
            'info_hash' => strtolower((string) $torrent->info_hash), // 40-hex
        ...
        // Ensure deterministic transport
        if (isset($params['info_hash']) && is_string($params['info_hash'])) {
            $params['info_hash'] = strtolower($params['info_hash']);
        }
        if (isset($params['peer_id']) && is_string($params['peer_id'])) {
            $params['peer_id'] = strtolower($params['peer_id']);
        }
    ...
    {
        // 40-char hex (20 bytes) – safe for query strings
        return strtolower(bin2hex(substr(hash('sha1', $seed, true), 0, 20)));
    }
