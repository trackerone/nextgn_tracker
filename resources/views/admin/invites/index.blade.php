...
<body>
    ...
    <nav>
        ...
        @foreach ($options as $value => $label)
            <a href="{{ route('admin.invites.index', array_filter(['status' => $value])) }}"
               class="{{ ($status ?? '') === $value ? 'pill-active' : 'pill-muted' }}">
                {{ $label }}
            </a>
        @endforeach
    </nav>
    ...
    <form method="POST" action="{{ route('admin.invites.store') }}">
        ...
        <label>
            Max uses
            <input type="number" name="max_uses" min="1" value="{{ old('max_uses', 1) }}" required>
        </label>
        <label>
            Expires at (optional)
        ...
        <tbody>
            @forelse ($invites as $invite)
                <tr>
                    ...
                    <td>{{ optional($invite->expires_at)->format('Y-m-d H:i') ?? 'Never' }}</td>
