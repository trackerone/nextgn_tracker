...
import {
  fetchDiscoveryRssSuggestions,
  type DiscoveryAggregateItem,
  type DiscoveryRssSuggestionCategory,
  type DiscoveryRssSuggestionsPayload,
} from '../../lib/discovery';

type SuggestionsState =
  | { status: 'loading' }
  | { status: 'error'; message: string }
  | { status: 'ready'; payload: DiscoveryRssSuggestionsPayload };

type SuggestionSection = {
  key: DiscoveryRssSuggestionCategory;
  title: string;
};
...
function hasSuggestions(payload: DiscoveryRssSuggestionsPayload): boolean {
...
export default function RssDiscoverySuggestions(): React.ReactElement {
  const [state, setState] = useState<SuggestionsState>({ status: 'loading' });

  useEffect(() => {
    let cancelled = false;

    void fetchDiscoveryRssSuggestions()
      .then((payload) => {
        if (!cancelled) {
      ...
      .catch(() => {
        if (!cancelled) {
          setState({
            status: 'error',
            message: 'RSS discovery suggestions are temporarily unavailable.',
          });
        }
      });
    ...
    if (!hasSuggestions(state.payload)) {
      return (
        <p className="rounded-xl border border-slate-800 bg-slate-950/40 p-3 text-sm leading-6 text-slate-400">
          No RSS discovery suggestions are available yet.
        </p>
      );
    }
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/20">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold text-white">RSS preset ideas</h2>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-400">
            Popular metadata signals that may help you decide which filters to type below. Suggestions are read-only and do not change the preset.
          </p>
        </div>
      ...
      <div className="mt-4">
        {state.status === 'loading' && (
          <div className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/40 p-3 text-sm text-slate-400" aria-live="polite">
            ...
            Loading RSS discovery suggestions...
          </div>
        )}
