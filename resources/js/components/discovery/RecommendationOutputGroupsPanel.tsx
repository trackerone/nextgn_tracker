...
export default function RecommendationOutputGroupsPanel(): React.ReactElement {
  ...
  const content = useMemo(() => {
    ...
    if (state.payload.recommendation_groups.length === 0) {
      return (
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-400">
                    Metadata output group only. No torrent picks, ordering metrics, or user history are exposed.
                  </p>
                </div>
              </div>
            </li>
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5 shadow-xl shadow-slate-950/20">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold text-white">Readonly metadata output groups</h2>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-400">
            System-wide metadata combinations prepared for future recommendation surfaces. This readonly preview does not personalize, prioritize, or expose concrete torrents.
          </p>
        </div>
