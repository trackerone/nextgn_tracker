# Documentation Index

Use this index to find the canonical document for each operational topic.

## Start here

- [Installation](INSTALLATION.md) — full local/test installation and first boot.
- [Stack baseline](STACK-BASELINE.md) — runtime/server/software requirements.
- [Frontend setup](FRONTEND-SETUP.md) — npm/Vite development and build flow.

## Production

- [Production operations](PRODUCTION-OPERATIONS.md) — deploy/update/runbook and troubleshooting.
- [Production hardening](security/production-hardening.md) — hardening controls and validation.
- [Security checklist](SECURITY-CHECKLIST.md) — pre-go-live checklist only.

## Operations runbooks

- [Backups and restore](operations/backups-and-restore.md)
- [Monitoring and alerting](operations/monitoring-and-alerting.md)
- [Process management](operations/process-management.md)
- [Upgrading](installation/upgrading.md)

## Architecture and functional flows

- [Architecture](ARCHITECTURE.md)
- [Architecture (repositories)](ARCHITECTURE-NEXTGN-REPOSITORIES.md)
- [Tracker engine](TRACKER-ENGINE.md)
- [Download flow](DOWNLOAD-FLOW.md)
- [Upload workflow](UPLOAD-WORKFLOW.md)
- [API contract](API_CONTRACT.md)
- [Security overview](SECURITY-OVERVIEW.md)
