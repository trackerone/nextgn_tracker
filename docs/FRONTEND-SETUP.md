...
The NextGN Tracker UI uses [Vite](https://vitejs.dev/) with the official Laravel plugin plus a React + Tailwind stack. Keep the backend running with `php artisan serve` (or Sail/Herd) while you work locally so Vite can proxy API calls during development.
...
npm install
```
This installs the Vite toolchain plus Tailwind CSS, shadcn/ui utilities, lucide-react, and the Laravel Vite plugin. Dependencies are defined in `package.json` and TypeScript is configured via `tsconfig.json`.
...
Blade templates must load these via `@vite(['resources/css/app.css','resources/js/app.tsx'])`. The forum UI expects session data to be exposed as `window.__APP__` (see `resources/views/welcome.blade.php`).
...
- Remove unused components/libs promptly so Vite bundles stay small and tree-shaking remains effective.
